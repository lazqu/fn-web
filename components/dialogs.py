...
def show_liquidation_dialog(ticker, current_price, p_shares, p_price, p_pos_type):
    with st.form("liq_dialog_form", clear_on_submit=True):
        ...
        exit_shares = st.number_input("청산할 수량 (주)", min_value=0.0, max_value=p_shares, value=p_shares, step=1.0, key="dlg_liq_shares")
        exit_price = st.number_input("청산 단가 ($)", min_value=0.0, value=current_price, step=0.01, key="dlg_liq_price")
...
def render_order_history_panel(ticker, pos_type, position_id):
    ...
    st.subheader("⌛ 최근 체결 이력 (정정/취소 가능)")
