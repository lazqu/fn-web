...
def render_purchase_inline_form(ticker, current_price, in_portfolio, p_shares=0.0, p_price=0.0, p_entry_reason="", p_pos_type="LONG", state_key="quick_active_form"):
    with st.form(f"purchase_inline_form_{ticker}", clear_on_submit=True):
        ...
        shares_add = st.number_input("진입 수량 (주)", min_value=0.0, value=0.0, step=1.0, key=f"dlg_purchase_shares_{ticker}")
        price_add = st.number_input("진입 단가 ($)", min_value=0.0, value=current_price, step=0.01, key=f"dlg_purchase_price_{ticker}")
        reason_in = st.text_area("상세 진입 근거 및 메모", value=p_entry_reason if in_portfolio else "", height=80, key=f"dlg_purchase_reason_{ticker}")
        
        c1, c2 = st.columns(2)
        with c1:
            pf_submit = st.form_submit_button("진입 실행", use_container_width=True, type="primary")
        with c2:
...
def render_liquidation_inline_form(ticker, current_price, p_shares, p_price, p_pos_type, state_key="quick_active_form"):
    with st.form(f"liq_inline_form_{ticker}", clear_on_submit=True):
        ...
        exit_shares = st.number_input("청산할 수량 (주)", min_value=0.0, max_value=p_shares, value=p_shares, step=1.0, key=f"dlg_liq_shares_{ticker}")
        exit_price = st.number_input("청산 단가 ($)", min_value=0.0, value=current_price, step=0.01, key=f"dlg_liq_price_{ticker}")
        exit_reason = st.text_area("청산 사유 / 기록", value="", height=80, key=f"dlg_liq_reason_{ticker}")
        
        c1, c2 = st.columns(2)
        with c1:
            liq_submit = st.form_submit_button("청산 실행", use_container_width=True, type="primary")
        with c2:
            cancel = st.form_submit_button("❌ 취소", use_container_width=True)
        ...
        if liq_submit:
            if exit_shares > 0:
                ...
                st.error("청산할 수량을 0보다 크게 입력해주세요.")

        if cancel:
            st.session_state[state_key] = None
            st.rerun()

def render_alert_inline_form(ticker, current_price, state_key="quick_active_form"):
    with st.form(f"alert_inline_form_{ticker}", clear_on_submit=True):
        st.markdown(f"##### 🎯 {ticker} 조건부 타겟 설정 (현재가: ${current_price:.2f})")
        cond_in = st.text_input("조건식 입력 (예: >= 150 또는 <= 50)", value=f">= {current_price:.2f}", key=f"dlg_al_cond_{ticker}")
        
        c1, c2 = st.columns(2)
        with c1:
            al_submit = st.form_submit_button("타겟 등록", use_container_width=True, type="primary")
        with c2:
            cancel = st.form_submit_button("❌ 취소", use_container_width=True)
...
def render_wl_pf_inline_form(ticker, current_price, state_key="quick_active_form"):
    with st.form(f"wl_pf_inline_form_{ticker}", clear_on_submit=True):
        st.markdown(f"##### 🚀 {ticker} 포지션 신규 진입 (현재가: ${current_price:.2f})")
        pos_in = st.selectbox("포지션", ["LONG", "SHORT"], key=f"dlg_wl_pos_sel_{ticker}")
        shares_in = st.number_input("진입 수량 (주)", min_value=0.0, value=10.0, step=1.0, key=f"dlg_wl_shares_{ticker}")
        price_in = st.number_input("진입 단가 ($)", min_value=0.0, value=current_price, step=0.01, key=f"dlg_wl_price_{ticker}")
        reason_in = st.text_area("진입 사유", value="", height=80, key=f"dlg_wl_reason_{ticker}")
        
        c1, c2 = st.columns(2)
        with c1:
            pf_add_submit = st.form_submit_button("포지션 진입 실행", use_container_width=True, type="primary")
        with c2:
            cancel = st.form_submit_button("❌ 취소", use_container_width=True)
